    ...
    private void MakeOneStepTowardsTheTarget()
    {
       ...
       // Debug.Log("shortestPath = " + string.Join(" , ", shortestPath));
        ...
        { // shortestPath contains both source and target.
            ...
            Vector3 targetPosition = tilemap.GetCellCenterWorld(nextNode);
            float step = speed * Time.deltaTime; // Calculate the distance to move this frame
            transform.position = Vector3.MoveTowards(transform.position, targetPosition, step); // Move towards the target position
        }
        else
        {
            atTarget = true;
